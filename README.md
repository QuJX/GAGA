...
This repository contains code and scripts for reproducing the **GAGA**. In this project, we use [EDM](https://arxiv.org/abs/2203.17003) as an example to plug-in GAGA algorithm. Because most of other 3D molecular generation baselines are constructed based on EDM, so it's easy to transfer EDM-GAGA to other baselines-GAGA.
...
The program will print the variance across all samples. It takes some time to run QM9 and GEOM dataset. Here we directly provide the stats of them:
...
Notes: the reason why we need the model here is the noising schedule is saved in the model. So this model path can direct to either original EDM weights or GAGA-trained weights.
...
It will take long time to run the statistical test on the whole dataset in step 2 and 3. Therefore, we provide the stats of QM9 and GEOM(random sampled 10%) here:
...
Note: from the orginal algorith, the timestep can be even earlier. However, because the randomness in noise sampling and statistical test, we utilized the GA timestep is little larger (100 steps) than original estimated timesteps. 
