...
parser.add_argument('--max_t', type=int, default=1000,
                    help='max_training_step')
parser.add_argument('--dataset_var', type=float, default=0.952,
                    help='max_training_step')
...
def main():
    ...
    for epoch in range(args.start_epoch, args.n_epochs):
        ...
        train_test.train_epoch(args, dataloaders['train'], epoch, model, model_dp, model_ema, ema, device, dtype,
                               ...
                               prop_dist, args.max_t)
        print(f"Epoch took {time.time() - start_epoch:.1f} seconds.")

        if epoch == 0 or epoch > 10 and epoch % args.test_epochs == 0:
            ...
            if not args.break_train_epoch:
                train_test.analyze_and_save(epoch, model_ema, nodes_dist, args, device,
                                            ...
                                            batch_size=20, max_t = args.max_t, dataset_var = args.dataset_var)
            nll_val = train_test.test(args, dataloaders['val'], epoch, model_ema_dp, device, dtype,
                                      property_norms, nodes_dist, partition='Val', max_t = args.max_t)
            nll_test = train_test.test(args, dataloaders['test'], epoch, model_ema_dp, device, dtype,
                                       property_norms, nodes_dist, partition='Test', max_t = args.max_t)
