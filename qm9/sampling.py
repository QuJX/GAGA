...
def pad_nodes(tensor, target_nodes):
    """
    Pads a (B, N, D) or (B, N) tensor with zeros so that N = target_nodes.
    """
    if tensor.dim() == 3:  # (B, N, F)
        B, N, _ = tensor.shape
        pad_size = target_nodes - N
        # print(tensor)
        if pad_size > 0:
...
def shortcut_sample(args, device, generative_model, dataset_info, prop_dist=None, nodesxsample=torch.tensor([10]), context=None,
        fix_noise=False, start_point = 300, sampling_step_size=1, shortcut = 'Gaussian', dataset_var = 1):
    
    max_n_nodes_dataset = dataset_info['max_n_nodes']  # this is the maximum node_size in QM9
    
    assert int(torch.max(nodesxsample)) <= max_n_nodes_dataset
    ...
    edge_mask = node_mask.unsqueeze(1) * node_mask.unsqueeze(2)
    diag_mask = ~torch.eye(edge_mask.size(1), dtype=torch.bool).unsqueeze(0)
    # print(diag_mask)  all 1s, except diagonal 0
    edge_mask *= diag_mask
    edge_mask = edge_mask.view(batch_size * max_n_nodes * max_n_nodes, 1).to(device)
    node_mask = node_mask.unsqueeze(2).to(device)

    # TODO FIX: This conditioning just zeros.
    # print(args.context_node_nf)  0

    context = None
    
    if args.probabilistic_model == 'diffusion':   # EnVariationalDiffusion, equivariant or non-equivariant is an argument called mode in arg.
        z = generative_model.sample_combined_position_feature_noise(batch_size, max_n_nodes, node_mask)
        v_t = generative_model.get_variance_t(dataset_var, start_point) # # 0.9261 for qm9_550
        z = z * torch.sqrt(v_t)
            
        x, h = generative_model.sample_shortcut(batch_size, max_n_nodes, node_mask, edge_mask, context, 
                                                fix_noise=fix_noise, z = z, start_point = start_point,
    ...
    one_hot, x, node_mask = (pad_nodes(tensor, max_n_nodes_dataset) for tensor in (one_hot, x, node_mask))
    return one_hot, charges, x, node_mask
