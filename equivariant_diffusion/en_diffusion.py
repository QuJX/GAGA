...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def get_z_t(self, x, h, node_mask, t):
        """Computes an estimator for the variational lower bound, or the simple loss (MSE)."""

        # Sample a timestep t.
        t_int = t_int = torch.full((x.size(0), 1), fill_value=t, device=x.device).float()
        s_int = t_int - 1
        ...
        return z_t, float(alpha_t.detach().cpu()[0]), float(sigma_t.detach().cpu()[0])
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always, max_t = 1000):
        """Computes an estimator for the variational lower bound, or the simple loss (MSE)."""
        ...
        # Sample a timestep t.
        t_int = torch.randint(
            lowest_t, max_t + 1, size=(x.size(0), 1), device=x.device).float()
        s_int = t_int - 1
        ...
        if t0_always:
            ...
            num_terms = max_t  # Since t=0 is not included here.
    ...
    def get_variance_t(self, variance_0, t):
        # Normalize t to [0, 1]. Note that the negative
        # step of s will never be used, since then p(x | z0) is computed.
        t = torch.ones([1]).to(self.dynamics.device) * t / self.T
        variance_0 = torch.ones([1]).to(self.dynamics.device) * variance_0

        # Compute gamma_s and gamma_t via the network.
        gamma_t = self.inflate_batch_array(self.gamma(t), variance_0)

        # Compute alpha_t and sigma_t from gamma.
        alpha_t = self.alpha(gamma_t, variance_0)
        sigma_t = self.sigma(gamma_t, variance_0)

        variance_t = alpha_t ** 2 * variance_0 + sigma_t ** 2
        
        return variance_t
    
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None, max_t = 1000):
        """
        ...
        if self.training:
            ...
            loss, loss_dict = self.compute_loss(x, h, node_mask, edge_mask, context, t0_always=False, max_t = max_t)
        else:
            # Less variance in the estimator, costs two forward passes.
            loss, loss_dict = self.compute_loss(x, h, node_mask, edge_mask, context, t0_always=True, max_t = max_t)
    ...
    def sample_p_zs_given_zt(self, s, t, zt, node_mask, edge_mask, context, fix_noise=False):
        ...
        # zs = torch.cat([diffusion_utils.remove_mean_with_mask(zs[:, :, :self.n_dims], node_mask),
        #      zs[:, :, self.n_dims:]], dim=2
        # )
        zs = diffusion_utils.remove_mean_with_mask(zs[:, :, :], node_mask)
        
        return zs
    
    def sample_p_zs_given_zt_ddim(self, s, t, zt, node_mask, edge_mask, context, fix_noise=False):
        """Samples from zs ~ p(zs | zt). Only used during sampling."""
        gamma_s, gamma_t = self.gamma(s), self.gamma(t)
        alpha_s, alpha_t = self.alpha(gamma_s, target_tensor=zt), self.alpha(gamma_t, target_tensor=zt)
        sigma_s, sigma_t = self.sigma(gamma_s, target_tensor=zt), self.sigma(gamma_t, target_tensor=zt)
        ...
        diffusion_utils.assert_mean_zero_with_mask(zt[:, :, :self.n_dims], node_mask)
        diffusion_utils.assert_mean_zero_with_mask(eps_t[:, :, :self.n_dims], node_mask)
        z_zero = (zt - sigma_t * eps_t)/alpha_t
        zs = alpha_s * z_zero + sigma_s * eps_t
        # Project down to avoid numerical runaway of the center of gravity.
        zs = torch.cat(
    ...
    def sample_p_zs_given_zt_no_wiener(self, s, t, zt, node_mask, edge_mask, context, fix_noise=False):
        """Samples from zs ~ p(zs | zt). Only used during sampling."""
        gamma_s = self.gamma(s)
        gamma_t = self.gamma(t)
        ...
        diffusion_utils.assert_mean_zero_with_mask(zt[:, :, :self.n_dims], node_mask)
        diffusion_utils.assert_mean_zero_with_mask(eps_t[:, :, :self.n_dims], node_mask)
        mu = zt / alpha_t_given_s - (sigma2_t_given_s / alpha_t_given_s / sigma_t) * eps_t/2
        zs = torch.cat(
    ...
    def sample_combined_position_feature_noise(self, n_samples, n_nodes, node_mask):
        ...
        z_x = utils.sample_center_gravity_zero_gaussian_with_mask(
            ...
            node_mask=node_mask)
        z_h = utils.sample_center_gravity_zero_gaussian_with_mask(
    ...
    def sample(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise=False):
        ...
        diffusion_utils.assert_mean_zero_with_mask(z[:, :, :], node_mask)
    ...
    @torch.no_grad()
    def sample_shortcut(self, n_samples, n_nodes, node_mask, edge_mask, context, fix_noise = False,
                        z = None, start_point = 300, sampling_step_size=1):
        """
        Draw samples from the generative model.
        """
        diffusion_utils.assert_mean_zero_with_mask(z[:, :, :self.n_dims], node_mask)
        # Iteratively sample p(z_s | z_t) for t = 1, ..., T, with s = t - 1.
        for s in reversed(range(0, start_point, sampling_step_size)):
            s_array = torch.full((n_samples, 1), fill_value=s, device=z.device)
            t_array = s_array + sampling_step_size
            s_array = s_array / self.T
            t_array = t_array / self.T
            z = self.sample_p_zs_given_zt(s_array, t_array, z, node_mask, edge_mask, context, fix_noise=fix_noise)
        # Finally sample p(x, h | z_0).
