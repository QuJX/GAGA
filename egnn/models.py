...
class EGNN_dynamics_QM9(nn.Module):
    ...
    def _forward(self, t, xh, node_mask, edge_mask, context):
        ...
        if torch.any(torch.isnan(vel)):
            print('Warning: detected nan, resetting EGNN output x to zero.')
            vel = torch.zeros_like(vel)
        if torch.any(torch.isnan(h_final)):
            print('Warning: detected nan, resetting EGNN output h to zero.')
            h_final = torch.zeros_like(h_final)

        if node_mask is None:
            vel = remove_mean(vel)
            h_final = remove_mean(h_final)
        else:
            vel = remove_mean_with_mask(vel, node_mask.view(bs, n_nodes, 1))
            h_final = remove_mean_with_mask(h_final, node_mask.view(bs, n_nodes, 1), assert_flag=False)
