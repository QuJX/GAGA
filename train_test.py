...
from qm9.sampling import sample_chain, sample, sample_sweep_conditional, shortcut_sample
import utils
...
def train_epoch(args, loader, epoch, model, model_dp, model_ema, ema, device, dtype, property_norms, optim,
                nodes_dist, gradnorm_queue, dataset_info, prop_dist, max_t):
    model_dp.train()
    model.train()
    nll_epoch = []
...
def Get_z_t(args, data, model_dp, device, dtype, property_norms, t_list):
    x = data['positions'].to(device, dtype)
    node_mask = data['atom_mask'].to(device, dtype).unsqueeze(2)
    edge_mask = data['edge_mask'].to(device, dtype)
    one_hot = data['one_hot'].to(device, dtype)
    ...
    check_mask_correct([x, one_hot, charges], node_mask)
    assert_mean_zero_with_mask(x, node_mask)
    assert_mean_zero_with_mask(one_hot, node_mask)

    h = {'categorical': one_hot, 'integer': charges}

    x, h, _ = model_dp.normalize(x, h, node_mask)
    ...
    # transform batch through flow
    for t in t_list:
        z_t, alpha_t, sigma_t = model_dp.get_z_t(x, h, node_mask, t)
        z_t_dict[t] = [z_t, alpha_t, sigma_t]
    return z_t_dict, data['atom_mask']
...
def test(args, loader, epoch, eval_model, device, dtype, property_norms, nodes_dist, 
         partition='Test', max_t = 1000):
    eval_model.eval()
    with torch.no_grad():
        nll_epoch = 0
        n_samples = 0
        ...
        for i, data in enumerate(loader):
            ...
            nll, _, _ = losses.compute_loss_and_nll(args, eval_model, nodes_dist, x, h,
                                                    node_mask, edge_mask, context, max_t = max_t)
            # standard nll from forward KL
...
def analyze_and_save(epoch, model_sample, nodes_dist, args, device, dataset_info, prop_dist,
                     n_samples=1000, batch_size=100, max_t = 800, dataset_var = 1):
    ...
    for i in range(int(n_samples/batch_size)):
        nodesxsample = nodes_dist.sample(batch_size)
        one_hot, charges, x, node_mask = shortcut_sample(args, device, model_sample, dataset_info, prop_dist,
                                                nodesxsample=nodesxsample, context=None, fix_noise=False, 
                                                start_point = max_t, sampling_step_size=1, sampling_name="DDPM", 
                                                shortcut = 'Gaussian', dataset_var = dataset_var)
